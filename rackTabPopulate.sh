...
read -p "What cluster (dPp) are you adding devices to? " dPp
...
#stay in same rpc if desired
	...
	while [[ $yn2 == "y" || $yn2 == "Y" ]]
	do
		read -p "Device Name (dId)? " dId
		read -p "Lower U? " lo
		read -p "Upper U? " hi
		read -p "Device Type (dT)? " dT
		read -p "Group (dPg)? " dPg
#ADD BITS TO VALIDATE INPUT?

#append line to file
		echo "{'dId':'$dId',   'r':$row,'p':'$pod','c':$cab,'lo':$lo,'hi':$hi,'dT':'$dT',    'dPp':'$dPp','dPg':'$dPg' }," >> ~/Documents/temprackfile.txt
		read -p "Add another device to this cabinet? " yn2
		yn2=${yn2:0:1}
	done
...
echo ">>  Formatted data is located in ~/Documents/temprackfile.txt <<"
